...
def main(dataset: str, backbone: str, weights_path: str):
    ...
    match backbone:
        ...
        case "densenet121":
            ...
            for k, v in weights.items():
                new_k = re.sub(r"^model\.", "model.features.", k)  # prepend "features."
                new_state_dict[new_k] = v

            model = DenseNet3DMonai(depth=121, n_classes=4, features_only=False)
            ...
            # Unfreeze only "class_layers"
            for name, param in model.named_parameters():
                if "class_layers" in name:
